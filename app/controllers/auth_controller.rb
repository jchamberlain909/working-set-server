class AuthController < ApplicationController
    ...
    def login
        ...
        if user && user.authenticate(password)
            render json: {success:true, id: user.id, name: user.name, email: user.email, token: generate_token(user) }
    ...
    def signup
        ...
        if @current_user.valid?
            render json: {success: true,id: @current_user.id, name: @current_user.name, email: @current_user.email, token: generate_token(@current_user)}
        else 
            error_message = ""
